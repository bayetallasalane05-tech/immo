...
try {
    ...
    // 1. On récupère uniquement les annonces de catégorie 'villa'
    $stmt = $db->prepare("SELECT * FROM annonces WHERE categorie = 'villa' ORDER BY date_publication DESC");
    $stmt->execute();
    ...
    $vid_query->execute(['villa']);
    $video_villa = $vid_query->fetchColumn();
...
.hero-overlay {
    ...
    /* --- GRILLE DE VILLAS --- */
    .villas-section { padding: 80px 0; background: #fdfdfd; }
...
<main>
    <section class="hero-category">
        <video autoplay muted loop playsinline>
            <source src="villas.mp4" type="video/mp4">
        </video>
        <div class="hero-overlay"></div>
        <div class="hero-content" style="position:relative; z-index:1;">
            <h1 class="display-3 fw-bold">Nos Villas d'Exception</h1>
            <p class="lead">Le luxe, le confort et l'intimité dans les plus beaux quartiers du Sénégal.</p>
        </div>
    </section>

    <section class="container villas-section">
        <div class="text-center">
            <h2 class="category-title">Découvrez nos opportunités</h2>
        </div>
        ...
        <?php else: ?>
            <div class="text-center py-5">
                ...
                <p class="mt-3 fs-5 text-muted">Aucune villa n'est disponible pour le moment.</p>
            </div>
        <?php endif; ?>
    </section>
</main>
